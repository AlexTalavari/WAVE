...
namespace WAVE.Website.Classes
{
    ...
    public sealed class QValueList : List<QValue>
    {
        ...
        /// </remarks>
        public QValueList(IEnumerable<string> values)
        {
            int ordinal = -1;
        ...
        {
            Predicate<QValue> criteria = item => IsCandidate(item.Name, candidates) && item.CanAccept;
            return Find(criteria);
        }
        ...
        {
            return candidates.Any(candidate => candidate.Equals(item, StringComparison.OrdinalIgnoreCase));
        }
